...
import { Search, SlidersHorizontal, Columns3, ChevronDown, X, Eye, UserPlus, Check } from "lucide-react";
import { ReferPatientModal } from "@/components/provider/patients/ReferPatientModal";
import {
  calcAge, getCareCoordinator, getLastVisit, getNextVisit,
  CARE_COORDINATORS, type PatientProfile,
} from "@/data/provider-patients";
import { PROVIDERS } from "@/data/providers";
import { cn } from "@/lib/utils";

type ColKey = "genderAge" | "primaryProvider" | "clinic" | "patientType" | "insurance" | "lastVisit" | "nextAppt" | "status";
...
const AGE_BUCKETS = [
  { value: "any", label: "Any age" }, { value: "u30", label: "Under 30" },
  { value: "30-50", label: "30 – 50" }, { value: "o50", label: "Over 50" },
];
...
export function PatientsPanel({
  patients, basePath, title, subtitle, referable = false, visitScope,
}: {
  ...
  subtitle: string;
  referable?: boolean;
  /** provider id to scope last/next visit lookups; "*" or undefined = any provider */
  visitScope?: string;
}) {
  const scope = visitScope ?? "*";
  const showPrimaryProviderCol = scope === "*";

  const OPTIONAL_COLS: { key: ColKey; label: string }[] = useMemo(() => [
    { key: "genderAge", label: "Gender / Age" },
    ...(showPrimaryProviderCol ? [{ key: "primaryProvider" as ColKey, label: "Primary Provider" }] : []),
    { key: "patientType", label: "Patient Type" },
    { key: "insurance", label: "Insurance" },
    { key: "lastVisit", label: "Last Visit" },
    { key: "nextAppt", label: "Next Appointment" },
    { key: "status", label: "Status" },
  ], [showPrimaryProviderCol]);

  const [search, setSearch] = useState<string>(() => {
    if (typeof window === "undefined") return "";
    return new URLSearchParams(window.location.search).get("q") ?? "";
  });
  const [visibleCols, setVisibleCols] = useState<Set<ColKey>>(new Set(showPrimaryProviderCol ? ["primaryProvider" as ColKey] : []));
  ...
  const activeFilterCount =
    (fCoordinator !== "any" ? 1 : 0) + (fStatus !== "any" ? 1 : 0) +
    (fInsurance !== "any" ? 1 : 0) + (fAge !== "any" ? 1 : 0) + (fUpcomingOnly ? 1 : 0);
  ...
  const rows = useMemo(() => {
    ...
    return patients
      .filter((p) => {
        ...
        if (fUpcomingOnly && !getNextVisit(p.id, scope)) return false;
        if (fAge !== "any") {
  ...
  }, [patients, search, fCoordinator, fStatus, fInsurance, fAge, fUpcomingOnly, scope]);
  ...
  return (
    <div className="p-6">
      <div className="flex items-start justify-between gap-4 mb-5">
        ...
        {referable && (
          <button onClick={() => setRefer(true)} data-tour="pt-refer"
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <div data-tour="pt-search" className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 w-full sm:w-72">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, MRN, email, phone…"
        ...
        <div className="relative" ref={colRef} data-tour="pt-columns">
          <button onClick={() => { setColMenu((o) => !o); setFilterMenu(false); }}
        ...
        <div className="relative" ref={filterRef} data-tour="pt-filters">
          <button onClick={() => { setFilterMenu((o) => !o); setColMenu(false); }}
            className={cn("flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium border",
              activeFilterCount > 0
            ...
            <div className="absolute left-0 top-11 z-30 w-72 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xl p-3 space-y-3">
              ...
              <FilterField label="Patient status">
                <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className={selCls}>
                  <option value="any">Any</option><option value="active">Active</option><option value="inactive">Inactive</option>
                </select>
              </FilterField>
              ...
              {activeFilterCount > 0 && (
                <button onClick={clearFilters} className="w-full text-xs font-medium text-brand-700 dark:text-brand-400 hover:underline text-left">Clear all filters</button>
              )}
            </div>
          )}
        </div>
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-left">
              <Th>Patient</Th><Th>Full Name</Th><Th>Date of Birth</Th>
              {visibleCols.has("genderAge") && <Th>Gender / Age</Th>}
              <Th>Mobile Phone</Th><Th>Email</Th>
              {visibleCols.has("primaryProvider") && <Th>Primary Provider</Th>}
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {rows.length === 0 ? (
              <tr><td colSpan={14} className="px-4 py-10 text-center text-sm text-slate-400">No patients match your search or filters.</td></tr>
            ) : rows.map((p) => <Row key={p.id} p={p} cols={visibleCols} basePath={basePath} scope={scope} />)}
          </tbody>
        </table>
      </div>
...
function Row({ p, cols, basePath, scope }: { p: PatientProfile; cols: Set<ColKey>; basePath: string; scope: string }) {
  const coordinator = getCareCoordinator(p.careCoordinatorId);
  const last = getLastVisit(p.id, scope);
  const next = getNextVisit(p.id, scope);
  const primary = PROVIDERS.find((x) => x.id === p.primaryProviderId);
  const href = `${basePath}/patients/${p.id}`;
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
      ...
      <td className="px-4 py-3">
        <Link href={href} className="font-medium text-brand-700 dark:text-brand-400 hover:underline whitespace-nowrap">{p.displayName}</Link>
      </td>
      ...
      {cols.has("primaryProvider") && (
        <td className="px-4 py-3 whitespace-nowrap">
          {primary ? (
            <span className="inline-flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: primary.color }} />{primary.displayName}
            </span>
      ...
      {cols.has("status") && (
        <td className="px-4 py-3">
          <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded capitalize", p.status === "active" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400" : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400")}>{p.status}</span>
        </td>
      )}
      <td className="px-4 py-3">
