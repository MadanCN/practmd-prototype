...
export default function ProviderSidebar() {
  ...
  const isActive = (href: string) =>
    href === BASE
      ...
      : pathname === href || (href !== `${BASE}/appointments` && pathname.startsWith(href));
    ...
    <aside className={cn(
      ...
      <div className="flex items-center justify-between h-[60px] px-4 border-b border-slate-800 shrink-0">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-violet-600 flex items-center justify-center shrink-0 text-white font-bold text-sm">P</div>
          {!collapsed && (
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="text-white font-semibold text-sm tracking-tight truncate">PractMD</span>
              <span className="text-[10px] font-medium text-violet-300 bg-violet-950 px-1.5 py-0.5 rounded shrink-0">Provider</span>
            </div>
          )}
        </div>
                ...
                {collapsed ? (
                  <Link href={item.children[0].href}
                    className={cn("flex items-center justify-center w-full px-2 py-2 rounded-lg text-sm font-medium transition-colors",
                      childActive ? "bg-violet-700 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800")}>
                  ...
                  <>
                    <button onClick={() => setApptOpen(o => !o)}
                      className={cn("w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors",
                        childActive ? "text-violet-400" : "text-slate-400 hover:text-white hover:bg-slate-800")}>
                      <item.icon className="w-[18px] h-[18px] shrink-0" />
                        ...
                        {item.children.map(child => (
                          <Link key={child.href} href={child.href}
                            className={cn("flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm font-medium transition-colors",
                              isActive(child.href) ? "bg-violet-700/80 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800")}>
          ...
          return (
            <Link key={item.label} href={item.href!}
              ...
              className={cn("flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors",
                active ? "bg-violet-700 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800",
      ...
      <div className="shrink-0 border-t border-slate-800 py-2 px-2 space-y-0.5">
        ...
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-3 py-2 mb-1 rounded-lg bg-slate-800/50">
            <div className="w-7 h-7 rounded-full bg-violet-600 flex items-center justify-center text-[10px] font-bold text-white shrink-0">SM</div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-slate-200 truncate">Dr. Sarah Mitchell</p>
              <p className="text-[10px] text-slate-500 truncate">Psychiatrist</p>
            </div>
          </div>
        )}
        {BOTTOM_NAV.map(item => {
