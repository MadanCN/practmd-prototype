...
import { Plus, Search, Pencil, PowerOff, Trash2, ListOrdered } from "lucide-react";
...
const SEED: CancellationReason[] = [
  { id: "1", name: "Patient Request", displayOrder: 1, isActive: true },
  { id: "2", name: "Provider Unavailable", displayOrder: 2, isActive: true },
  { id: "3", name: "Emergency", displayOrder: 3, isActive: true },
  { id: "4", name: "Weather", displayOrder: 4, isActive: true },
  { id: "5", name: "Insurance Issue", displayOrder: 5, isActive: true },
  { id: "6", name: "Scheduling Conflict", displayOrder: 6, isActive: false },
];
...
export default function CancellationReasonsScreen() {
  const [reasons, setReasons] = useState<CancellationReason[]>(SEED);
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-blue-50 dark:bg-blue-950 flex items-center justify-center">
            <ListOrdered className="w-[18px] h-[18px] text-blue-500" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">Cancellation Reasons</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Manage reasons available when cancelling an appointment</p>
          </div>
        </div>
        <button
          onClick={openAdd}
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-sm text-slate-400">
                    {query ? `No reasons matching "${query}"` : "No cancellation reasons yet. Click Add to create one."}
                  </td>
                </tr>
              )}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xl p-6 w-80">
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 mb-2">Delete Cancellation Reason</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              This will permanently remove the cancellation reason. This action cannot be undone.
            </p>
      ...
      <Drawer
        ...
        title={editing ? "Edit Cancellation Reason" : "Add Cancellation Reason"}
        description={editing ? `Editing: ${editing.name}` : "Create a new cancellation reason"}
        footer={
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Inactive reasons won&apos;t appear in the cancellation flow</p>
            </div>
            <Toggle
