...
import {
  CC_TOURS, ccTourIdForPath, isCcTourMainRoute, ccTourSeenKey,
  type CcTourPageId, type CcTourDef,
} from "@/lib/cc-tours";
...
export const useCcTour = () => useContext(TourContext);

function seen(id: CcTourPageId) {
  try { return !!window.localStorage.getItem(ccTourSeenKey(id)); } catch { return false; }
}
function markSeen(id: CcTourPageId) {
  try { window.localStorage.setItem(ccTourSeenKey(id), "seen"); } catch { /* private mode */ }
}

export function TourProvider({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const pageId = ccTourIdForPath(pathname);
  const def = pageId ? CC_TOURS[pageId] : null;
  ...
  // effect body.
  useEffect(() => {
    if (!pageId || !isCcTourMainRoute(pathname) || phase !== "idle") return;
